...
/*   Updated: 2019/10/04 22:45:42 by kradoste         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_width(char *str, t_printf *p, int max)
{
	...
	{
		if (pf_strchr("123456789", str[i]) && str[i - 1] != '.')
			p->width = pf_atoi(&str[i]);
		i++;
	}
}
...
void	parse_all(char *str, t_printf *p, int max)
{
	...
	while (str[i] && i < max)
	{
		...
		(str[i] == '.') && (p->dot = 1);
		(str[i] == ' ') && (p->space = 1);
		(!pf_strchr("123456789.", str[i - 1]) && str[i] == '0')
			&& (p->zero = 1);
		(str[i] == '.' && pf_strchr("0123456789", str[i + 1]))
			&& (p->pre = pf_atoi(&str[i + 1]));
		(str[i] == '.' && !pf_strchr("0123456789", str[i + 1]) && p->pre == -1)
			&& (p->pre = 0);
		i++;
	}
}
